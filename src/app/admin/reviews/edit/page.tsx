...
import { useReview, updateReview } from "@/hooks";
...
function AdminEditReviewPageContent() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const typeParam = searchParams.get("type") as ReviewType | null;
  const slugParam = searchParams.get("slug");

  const { data: review, loading, error: fetchError } = useReview(typeParam, slugParam);

  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const [reviewType, setReviewType] = useState<ReviewType>(typeParam || "forex");
  ...
  useEffect(() => {
    if (review) {
      ...
      setReviews(review.reviews || 0);
      setDescription(review.description || "");
      setUrlSite(review.url_site || "");
      setLogoUrl(review.logo || "");
      setLogoBg(review.logo_bg || "bg-black");
      setTags(review.tags || []);
      setPros(review.pros || review.advantages || []);
      setCons(review.cons || review.disadvantages || []);
      setStats(review.stats || []);
      setTerms(review.terms || []);
      setKeyInfo(review.key_info || []);
      setContentHtml(review.content || "");
    }
  }, [review]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!typeParam || !slugParam) {
      setError("Type and slug are required");
      return;
    }
    ...
    try {
      const reviewData = {
        ...
        reviews,
        description,
        url_site: urlSite || null,
        logo: logoUrl || null,
        logo_bg: logoBg || null,
        tags: tags.length > 0 ? tags : null,
        stats: stats.filter((s) => s.title && s.value).length > 0 ? stats.filter((s) => s.title && s.value) : null,
        terms: terms.filter((t) => t.label && t.value).length > 0 ? terms.filter((t) => t.label && t.value) : null,
        key_info: keyInfo.filter((k) => k.label && k.value).length > 0 ? keyInfo.filter((k) => k.label && k.value) : null,
        ...
        advantages: pros.length > 0 ? pros : null,
        disadvantages: cons.length > 0 ? cons : null,
      };

      // Call API to update review
      await updateReview(typeParam, slugParam, reviewData);
      ...
      <div className="space-y-6">
        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-6">
          <p className="text-red-700 dark:text-red-400">
            {fetchError ? `Error loading review: ${fetchError.message}` : "Review not found"}
          </p>
          <Link
            href="/admin/reviews"
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
            Edit broker/exchange/prop firm review. Detailed content is edited using the Lexical editor below.
          </p>
        </div>
        <Link
          href="/admin/reviews"
        ...
        <div className="space-y-4">
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-6">
            ...
            <div className="rounded-xl overflow-hidden bg-gray-50 dark:bg-gray-900">
              <LexicalEditor onChange={setContentHtml} initialHtml={review.content || undefined} rawHtmlFromDb={review.content || undefined} />
            </div>
          </div>
        </div>
      </form>
